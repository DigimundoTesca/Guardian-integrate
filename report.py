...
def sendmail():
    fromaddr = "jonathanbasilio24@outlook.com"
    ...
    part.set_payload(open("C:/Users/Basilio/Documents/Digimundo/guard2/status.xls", "rb").read())
    ...
    mailServer.starttls()
    mailServer.ehlo()
    mailServer.login(fromaddr, "jackhammer24")
    text = msg.as_string()
...
def create_report(values):
    ...
    wb = xlwt.Workbook()
    ws = wb.add_sheet('Status ICE')
    i = 1
    fields = ['Software version', 'Software CRC', 'Reference conditions', 'Temperature units', 'Default temperature',
              'Pressure units',
              'Default pressure', 'Delivery units', 'Total decimal places', 'Fluid type', 'Display T/O (min)',
              'Compensation method',
              'Pump Delay (min)', 'Trailer number',
              'Turbine serial number', 'ICE serial number', 'Meter size', 'K-factor method', 'Average K-factor',
              'Frequency[ 1]',
              'K-factor[ 1]', 'Frequency[ 2]', 'K-factor[ 2]', 'Frequency[ 3]', 'K-factor[ 3]', 'Frequency[ 4]',
              'K-factor[ 4]',
              'Frequency[ 5]', 'K-factor[ 5]', 'Last calibration date:', 'Next calibration date:',
              'Pulse Security Function:',
              'Pulse Error Max Percent:'
              ]
